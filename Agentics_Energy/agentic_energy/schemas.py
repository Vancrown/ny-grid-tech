...
class EnergyDataRecord(BaseModel):
    ...
    decisions: Optional[float] = Field(
        None,
        description="Decision taken at each time step by the battery - charge (+1), discharge (-1), idle (0)",
    )
...
class BatteryParams(BaseModel):
    ...
    soc_init: float = Field(
        ...
        ge=0,
        le=1,
        description="Initial State of Charge (SoC) as fraction of capacity",
    )
    soc_min: float = Field(
        0.0,
        ge=0,
        le=1,
        description="Minimum State of Charge (SoC) as fraction of capacity",
    )
    soc_max: float = Field(
        1.0,
        ge=0,
        le=1,
        description="Maximum State of Charge (SoC) as fraction of capacity",
    )
...
class SolveResponse(BaseModel):
    ...
    discharge_MW: Optional[List[float]] = Field(
        None, description="Battery discharge schedule in MW"
    )
    import_MW: Optional[List[float]] = Field(
        None, description="Grid import schedule in MW"
    )
    export_MW: Optional[List[float]] = Field(
        None, description="Grid export schedule in MW"
    )
    soc: Optional[List[float]] = Field(
        None, description="State of Charge (SoC) over time"
    )
    ...
    confidence: Optional[List[float]] = Field(
        None, description="Confidence level of each decision (0 to 1)"
    )
...
class ReasoningRequest(BaseModel):
    ...
    solve_request: SolveRequest = Field(
        description="Original solve request with battery parameters and inputs"
    )
    solve_response: SolveResponse = Field(
        description="Solver response containing decisions and results"
    )
    timestamp_index: int = Field(description="Index of the decision to explain")
    context_window: int = Field(
        default=6,
        description="Number of timesteps before/after to consider for context",
    )
...
class ReasoningResponse(BaseModel):  # all to optional, and assign to None if possible
    ...
    explanation: Optional[str] = Field(
        None, description="Detailed natural language explanation of the decision"
    )
    key_factors: Optional[List[str]] = Field(
        default_factory=list, description="Key factors that influenced the decision"
    )
    confidence: Optional[float] = Field(
        None, ge=0, le=1, description="Confidence in the explanation (0-1)"
    )
...
class PlotResponse(BaseModel):
    ...
    caption: Optional[str] = Field(
        None, description="Short description of what the plot shows"
    )
