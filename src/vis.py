...
# ---------- Tool ----------
def plot_price_forecast(prices, dt_hours, out_path):
    """
    ...
    out_dir = os.path.dirname(out_path) if out_path else "plots"
    if out_dir and not os.path.exists(out_dir):
        os.makedirs(out_dir, exist_ok=True)

    out_path = out_path or os.path.join(out_dir, "price_forecast.png")
    ...
    return dict(image_path=out_path, caption=caption)


def plot_price_soc(prices, capacity, soc, decision, out_path):
    """
    prices - prices_buy
    ...
    soc = np.array(soc)  # length T+1
    decisions = np.array(decision) if decision else None
    T = len(prices)
    hours = np.arange(T)
    ...
    base_path = out_path or os.path.join(out_dir, "battery_schedule.png")
    ...
    tick_markers = []
    (soc_line,) = ax2.plot(
        [], [], "-o", linewidth=2, markersize=5, color="tab:purple", label="SoC"
    )
    ...
    def animate(frame):
        ...
        for t in range(frame + 1):
            ...
            if a > 0:  # charge → green stem up
                ...
                marker = ax1.plot(
                    [t], [p + tick_length], marker="^", ms=6, color="tab:green"
                )[0]
                ...
                marker = ax1.plot(
                    [t], [p - tick_length], marker="v", ms=6, color="tab:red"
                )[0]
                tick_lines.append(stem)
                tick_markers.append(marker)
    ...
    anim = animation.FuncAnimation(
        fig, animate, init_func=init, frames=T, interval=200, blit=False, repeat=True
    )

    # Add legend
    price_up = Rectangle(
        (0, 0), 1, 1, color="tab:green", alpha=0.7, label="Bull candle"
    )
    price_dn = Rectangle((0, 0), 1, 1, color="tab:red", alpha=0.7, label="Bear candle")
    stem_up = plt.Line2D(
        [0], [0], color="tab:green", marker="^", linestyle="None", label="Charge"
    )
    stem_dn = plt.Line2D(
        [0], [0], color="tab:red", marker="v", linestyle="None", label="Discharge"
    )
    ax2.legend(
        [price_up, price_dn, stem_up, stem_dn, soc_line],
...
def plot_arbitrage_explanation(
    prices,
    capacity,
    soc_min,
    soc_max,
    ...
    base_path = out_path or os.path.join(out_dir, "arbitrage_explanation.png")
    out_path = (
        base_path.replace(".png", "_explanation.png")
        if "explanation" not in base_path
        else base_path
    )
    ...
    for i, price in enumerate(prices):
        if price <= low_threshold:
            ax1.axvspan(
                hours[i] - 0.4, hours[i] + 0.4, alpha=0.25, color="green", zorder=1
            )
        elif price >= high_threshold:
            ax1.axvspan(
                hours[i] - 0.4, hours[i] + 0.4, alpha=0.25, color="red", zorder=1
            )
    ...
    ax2.axhline(
        ...
        linestyle="--",
        alpha=0.5,
        label=f"Max Discharge ({dmax_MW} MW)",
    )
    ax2.axhline(
        y=-cmax_MW,
        color="green",
    ...
    ax3.axhline(
        ...
        linestyle="--",
        alpha=0.7,
        label=f"Min SoC ({soc_min:.1f} MWh)",
    )
    ax3.axhline(
        y=soc_max,
        color="darkblue",
        ...
        linestyle="--",
        alpha=0.7,
        label=f"Max SoC ({soc_max:.1f} MWh)",
    )
    ...
    round_trip_efficiency = (
        (total_discharge_energy / total_charge_energy * 100)
    ...
    caption = (
        ...
        f"Total discharged: {total_discharge_energy:.2f} MWh, Objective cost: ${objective_cost:.2f}."
    )
